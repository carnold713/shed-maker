...
- **Single model:** edit `BuildingModel` via commands in `lib/model/*.ts` (`commands`, `zones`, `interiorDoors`, `leanTos`, `electrical`, `drainage`); everything else is derived (`lib/framing`, `lib/geometry`, `lib/interior`, `lib/electrical`, `lib/plumbing`, `lib/bom`, `rules/`). Never store derived data.
