...
- **PBR materials (ADR-0018 addendum):** procedural wood, ribbed steel, board-and-batten, stone, concrete, gravel, grass and rubber texture sets (albedo, normal, roughness) at exact scale; merged per-material geometry with per-face UVs in feet replaces the instanced unit boxes; plain white mode turns it off.
- **Looks (ADR-0018):** cupola with weathervane sized to the ridge, bracketed awnings over doors, trim styles (flat, wide, craftsman), stone / board / steel wainscot, gooseneck and lantern door lights, and a one-click "Classic barn look"; all drawn in 3D, priced, sequenced, with two advisory rules.
- **Walk inside (ADR-0012 addendum):** first-person camera at a person's eye height (default 6'), drag to look, W A S D to walk, mini overhead map in the corner of the 3D view to drop in anywhere.
- **Fence builder (ADR-0017 addendum):** free fence lines drawn corner by corner on the plan or straight on the satellite map, open or closed (acres inside), corners drag, gates, takeoff, 3D and the site sheet; the Site step pane toggles Plan | Map; runs can be added on the map too; right-click or "Put the barn here" moves the barn anywhere on the map.
- **Runs, fencing and the Site step (ADR-0017, Site & fencing agent hired):** `model.runs[]` hung on pens or walls, sized per head from `docs/research/runs-and-fencing.md`; fencing takeoff with shared lines counted once; seven rules; grass, posts, mesh and gates in 3D; Run tool, auto runs, run inspector and menu; a Site step whose stage shows the barn and runs to scale on satellite imagery (Leaflet; tiles and geocoding proxied through `/api/tiles` and `/api/geo/search` — Google with `GOOGLE_MAPS_API_KEY`, Esri + Nominatim without). Drag the barn on the map, turn it with the handle. Site plan sheet A0 with the fence and gate schedules; fencing in the estimate and the build sequence.
- **Sliding aisle door (`aisleSlide`):** default inside door for aisles and open floor, sized to the aisle; lets an aisle be closed off from the next aisle or space (owner's entry "airlock").
