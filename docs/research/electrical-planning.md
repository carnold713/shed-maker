...
- **Switch legs**: the unswitched feed runs panel → first switch box → next switch box (→ any device with no switch); from each switch box the switched conductor runs out to only the lights that switch controls, daisy-chained. Lights on different switches never share a wire directly even when they share a breaker; the tool draws the feed in the circuit colour and each switch leg from its switch, and counts both in the takeoff (`lib/electrical/derive.ts` `RouteSegment.kind`).
