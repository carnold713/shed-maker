...
A new interior type, **`aisleSlide`** (sliding aisle door), is the default for aisles and open floor: a solid track-hung leaf sized to the edge it sits on (4'–12', a foot clear each side, 8' tall) so an aisle can be closed off from the next space — an entry vestibule ("airlock"), or a wing. Same track hardware as a stall door, two track sections.
